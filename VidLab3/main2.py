...
def detekcija_i_matchovanje(slika1, slika2, ratio_thresh=0.7, scale=1.0):
    ...
    # FLANN matcher koji koristi KD-tree; Fast Library for Approximate Nearest Neighbors; flann je automatski optimizovan - dizajniran da sam bira najbolje parametre radi brzine
    ...
    # Lowe-ov Ratio test - ovaj test uporedjuje stepen slicnosti(matematicku distancu) - uporedjuje deskriptore
    for m_n in mecovi:
        if len(m_n) < 2:
            continue
...
# homografija - matrica koja opisuje kako se jedna ravan(slika) transformise u drugu
# ransac algoritam(random sample consensus)
...
# mapira svaku sliku prema koordinatama srednje slike)
